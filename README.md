# Training Log PWA v0.11.5

個人向けのトレーニング記録PWAです。v0.11.5では、Homeのトレーニング入力画面に未保存時だけ表示されるフローティング保存ボタンを追加しています。v0.11.xで導入した Fullbody 区分、1日複数セッション、Drive差分確認、レスポンシブな分析表示も継続しています。
...
- Homeのトレーニング入力画面だけに、画面右下固定のフローティング「💾 保存」ボタンを追加。
- 入力内容が未保存のときだけフローティング保存ボタンを表示し、保存完了後は自動的に非表示。再度入力を変更すると再表示。
- 画面ロックやスクロール位置に関係なく、その場からトレーニングを保存可能。iPhoneのSafe Areaを考慮してホームインジケータと重ならない位置に配置。
- 既存の画面下部「トレーニングを保存」ボタンは残し、従来の操作方法も維持。
