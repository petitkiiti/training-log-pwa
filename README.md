# Training Log PWA v0.11.8

個人向けのトレーニング記録PWAです。v0.11系では Fullbody 区分、1日複数セッション、Google Drive差分確認、レスポンシブな分析グラフ、固定ヘッダー、フローティング保存ボタンなどを追加しています。v0.11.6では、日をまたいだ未保存トレーニングを元の日付のまま復元・保存できるようにしました。
...
- iPhone Safari / PWAで開始・終了の時刻入力欄がグリッド幅を超えて重なる問題を修正。
- 時刻入力を専用ラッパー内に固定し、通常入力画面と履歴編集画面の両方で2列レイアウトを維持。

## v0.11.5

- Homeのトレーニング入力画面に、未保存時のみ表示される右下固定のフローティング保存ボタンを追加。
- 保存完了後はフローティング保存ボタンを非表示にし、再編集すると再表示。
- ノート・種目管理・履歴・分析ではフローティング保存ボタンを表示しない。
- iPhoneのSafe Areaを考慮して配置。


## v0.11.6

- 未保存トレーニングの日付を入力開始時の日付のまま保持し、日付変更後に保存しても翌日へ自動変更されないよう修正。
- 日をまたいで未保存ドラフトが残っている場合、次回起動時に元の日付の記録として復元するか確認するよう変更。
- 復元した過去日の未保存トレーニングを編集中は、元の日付で保存されることをHome画面に表示。
- 未保存ドラフトは従来どおりGoogle Driveバックアップ／差分比較の対象外で、正式保存後に通常のバックアップ対象となる。

## v0.11.7

- トレーニング保存時、開始時間または終了時間が未入力なら警告を表示。
- 各種目の各条件について「重量 / 負荷」「回数」「セット数」の未入力を確認し、該当項目を警告に一覧表示。
- 未入力があっても、確認ダイアログで「そのまま保存」または「入力に戻る」を選択可能。
- 食事・種目所感・全体所感などの任意項目は未入力チェックの対象外。


## v0.11.8

- 保存時の未入力警告をスマホ向けに簡潔化。
- 開始時間または終了時間が未入力の場合は「開始終了時間が未入力です」と表示。
- 各種目の重量 / 負荷・回数・セット数のいずれかに未入力がある場合は「入力の完了していない種目があります」と表示。
- 未入力箇所を条件単位で大量に列挙せず、該当する警告だけを最大2行で表示。
- 未入力があっても従来どおり、そのまま保存するか入力に戻るかを選択可能。
